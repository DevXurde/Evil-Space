...
class Game():
    ...
    def movement(self):
        ...
        if keys_pressed[K_a] and self.player.x > 0:
            self.player.x -= self.player_speed

        if keys_pressed[K_d] and self.player.x < width - player_img.get_height() - 10:
            self.player.x += self.player_speed

        if keys_pressed[K_w] and self.player.y > 0:
            self.player.y -= self.player_speed

        if keys_pressed[K_s] and self.player.y < height - player_img.get_height() - 10:
            self.player.y += self.player_speed
